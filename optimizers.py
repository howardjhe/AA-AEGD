...
def aegd(x0, max_iters, step, f, proj=lambda x: x, c=1, linesearch=False, max_step=1.,
        ls_ratio=2, ls_adapt=True, verbose=False, verbskip=1, max_runtime=10, stop_eps=0.):
...
def AA_AEGD(init, max_iters, depth, kernel, loss, step, c=1, norm_type=2, reg=1e-10,
           verbose=False, verbskip=1, max_runtime=10, stop_eps=0.):
    """
    To be added
    """
    ...
    y_ext = kernel.gradient(x)  # y0
    r = np.sqrt(Fx[0] + c) # r0
    v = gradx / (2 * np.sqrt(Fx[0]+c)) # v0
    gx = y_ext - 2 * step * r * v  # g(y0)
    ...
    for idx in range(max_iters - 1):
        ...
        v = gradx / (2 * np.sqrt(Fx[idx + 1]+c))
        r = r / (1 + 2 * step * v**2)
        gx = kernel.gradient(x) - 2 * step * r * v
...
def AA_AEGD_on(init, max_iters, depth, kernel, loss, step, c=1, norm_type=2, reg=1e-10,
           verbose=False, verbskip=1, max_runtime=10, stop_eps=0.):
    """
    To be added
    """
    ...
    for idx in range(max_iters - 1):
        ...
        k += 1
        if k % depth == 0:
            y_test = AA_least_squares(R[-mk-1:], Gx[-mk-1:], mk, reg)
            x_test = kernel.prox_map(-y_test)
        else:
            # y_test = x_prox
            y_test = gx
...
def AA_BPG_on(init, max_iters, depth, kernel, loss, step, norm_type=2, reg=1e-10,
           verbose=False, verbskip=1, max_runtime=10, stop_eps=0.):
    """
    To be added
    """
